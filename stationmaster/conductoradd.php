...
/**
 * @Author: indran
 * @Date:   2018-11-25 10:19:55
 * @Last Modified by:   indran
 * @Last Modified time: 2018-11-25 10:50:02
 */
...
if(isset($_POST['submit-btn'])){
	...
	$stmnt=" SELECT * FROM conductor WHERE conductorid= '" . $_POST['conductorid'] ."' OR license= '" . $_POST['license'] ."' ";
	$result = $db->display( $stmnt);
	if( $result ){
		$message [0] = 2;
		$message [1] = ' conductor is already exists'; 
	} else {
		...
		$istrue=insertInToTable( 'conductor', $_POST,  $db);
		if($istrue){
					//$message=' added!';
			$message [0] = 1;
...
<div class="card">
	<div class="card-body">
		...
		<form  id="addevent"  action="" method="post" class="form-horizontal borderd-row" align="center" data-parsley-validate >
			...
			<center>	<h3 class="h3 mb-3 font-weight-normal danger-text">Add conductor</h3></center>
			...
			<div class="form-group row">
				...
				<div class="col-sm-9">
					<input type="text" class="form-control text-success" name="conductorid" placeholder="Conductor ID" data-parsley-required="true"   value="<?php
					try {
						$keyge = "C_" . Date('Y') . "_"; 
						$keyge .= rand(100,999)."";
						$result = selectFromTable( 'COUNT(*) AS count ', 'conductor' , "1", $db);
